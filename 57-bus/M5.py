...
def train_per_step(fs, g):
    ...
    gradients = [gradients1[x] + si_coff * gradients2[x] + 0.001 * gradients3[x] + si_coff * gradients4[x]
                 + si_coff * gradients5[x] for x in range(len(gradients2))]
    # 把梯度和变量进行绑定
    grads_and_vars = zip(gradients, model.trainable_variables)
    # 进行梯度更新
    my_opt.apply_gradients(grads_and_vars)
    loss_value = loss_value1 + si_coff * (loss_value3 + loss_value4)

    return loss_value, loss_value1, loss_value3, loss_value4, loss_value2, loss_value5
...
if __name__ == '__main__':
    ...
    for epoch in range(1000):
        ...
        for fs in data_generator(trainData, batch_size=batch_size):
            loss_value, loss_value1, loss_value3, loss_value4, loss_value2, loss_value5= train_per_step(fs, g)
            ...
            epoch_loss_avg_d.append(loss_value4.numpy())
        ...
        csv_writer.writerow([epoch, np.mean(epoch_loss_avg_total), np.mean(epoch_loss_avg_mse), np.mean(epoch_loss_avg_g),
                            np.mean(epoch_loss_avg_d), np.mean(epoch_loss_avg_j), np.mean(epoch_loss_avg_br),
                             np.mean(test_loss), np.mean(test2_loss)])
        print("Epoch {:03d}: Loss_total: {:.6f}, Loss_mse: {:.6f},Loss_g: "
              "{:.6f}, Loss_d: {:.6f}, Loss_j: {:.6f}, Loss_br: {:.6f},test1_loss:{:.6f}, test2_loss:{:.6f} ".format(epoch,
                                                                    np.mean(epoch_loss_avg_total),
                                                                    np.mean(epoch_loss_avg_mse),
                                                                    np.mean(epoch_loss_avg_g),
                                                                    np.mean(epoch_loss_avg_d),
