...
def train_per_step(fs, g):
    ...
    return loss_value, loss_value1, loss_value3, loss_value2, loss_value5, loss_value4
...
if __name__ == '__main__':
    ...
    csv_writer.writerow(["epoch", "total_loss", "mse_loss", "gradient_loss", "dis_loss", "j_loss", "d_loss", "test1_loss", "test2_loss"])
    losses = []
    ...
    for epoch in range(200):
        ...
        for fs in data_generator(trainData, batch_size=batch_size):
            loss_value, loss_value1, loss_value3, loss_value2, loss_value5, loss_value4 = train_per_step(fs, g)
            ...
            epoch_loss_avg_d.append(loss_value2.numpy())
        ...
        csv_writer.writerow([epoch, np.mean(epoch_loss_avg_total), np.mean(epoch_loss_avg_mse), np.mean(epoch_loss_avg_g),
                             np.mean(epoch_loss_avg_j), np.mean(epoch_loss_avg_d), np.mean(epoch_loss_avg_br),
                             np.mean(test_loss), np.mean(test2_loss)])
        print("Epoch {:03d}: Loss_total: {:.6f}, Loss_mse: {:.6f},Loss_g: "
              "{:.6f}, Loss_j: {:.6f}, Loss_j: {:.6f}, Loss_br: {:.6f},test1_loss:{:.6f}, test2_loss:{:.6f} ".format(epoch,
